            ...
            switch (summon_prop->Title)                     // better from known way sorting summons by AI types
            {
                case UNITNAME_SUMMON_TITLE_NONE:
                {
                    ...
                    // Snake trap exception
                        ...
                        summonResult = DoSummonSnakes(eff_idx);
...
bool Spell::DoSummonPet(SpellEffectIndex eff_idx)
{
    ...
    for (int32 count = 0; count < amount; ++count)
    {
        ...
        uint32 pet_number = sObjectMgr.GeneratePetNumber();
        if (!creature->Create(map->GenerateLocalLowGuid(HIGHGUID_PET), pos, cInfo, pet_number))
        {
        ...
        // set timer for unsummon
        if (m_duration > 0)
            creature->SetDuration(m_duration);

        creature->SetOwnerGuid(m_caster->GetObjectGuid());
        ...
        creature->SetCreatorGuid(m_caster->GetObjectGuid());
        ...
        // creature->SetName("");                          // generated by client

        map->Add((Creature*)creature);

        m_caster->SetPet(creature);
...
void Spell::EffectSummonPet(SpellEffectIndex eff_idx)
{
    ...
    if (m_caster->GetTypeId() == TYPEID_PLAYER && NewSummon->getPetType() == SUMMON_PET)
    {
        ...
        if (NewSummon->GetEntry() == 26125)
            NewSummon->setPowerType(POWER_ENERGY); // He must have energy bar instead of mana
