...
void Spell::prepareDataForTriggerSystem()
{
    ...
    if (!m_canTrigger)                                      // Exceptions (some periodic triggers)
    {
        switch (m_spellInfo->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_HUNTER:
                ...
                if (m_spellInfo->IsFitToFamilyMask(UI64LIT(0x0100000000000000), 0x00000200))
                    m_canTrigger = true;
                break;
    ...
    if (m_spellInfo->SpellFamilyName == SPELLFAMILY_HUNTER && (m_spellInfo->SpellFamilyFlags & UI64LIT(0x0000200000002008) || m_spellInfo->SpellFamilyFlags & UI64LIT(0x00064000)))
...
void Spell::DoSpellHitOnUnit(Unit* unit, uint32 effectMask)
{
    ...
    if (speed && (
                ...
                unit->IsImmuneToSpell(m_spellInfo, unit == realCaster)) && !(m_spellInfo->Attributes & SPELL_ATTR_UNAFFECTED_BY_INVULNERABILITY))
    {
        if (realCaster)
    ...
    for (int i = 0; i < MAX_EFFECT_INDEX; ++i)
    {
        ...
        switch (m_spellInfo->Effect[i])
        {
            ...
            case SPELL_EFFECT_DUMMY:
            {
                ...
                else if (m_spellInfo->SpellFamilyName == SPELLFAMILY_DEATHKNIGHT && m_spellInfo->SpellFamilyFlags & UI64LIT(0x2000)) // Death Coil (DeathKnight)
                {
                    Unit* target = m_targets.getUnitTarget();
